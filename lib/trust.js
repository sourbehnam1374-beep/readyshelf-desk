...
export const MUST_NOT_SHIP = [
  "“AI writes your channel.” / “AI in your voice” / style cloning / personality theater",
...
export const SUCCESS_CRITERIA = [
  ...
  "Do this 3 times with zero unapproved publishes.",
  "Stranger path: /start → Demo → Buy Desk · ⭐500 (desk30).",
...
export function freezePost(post, text) {
  if (!post || !post.id) {
  ...
  const frozen = String(text || "").trim();
  if (!frozen) {
    const err = new Error("text (frozen) is required");
    err.status = 400;
    throw err;
  }
  if (post.status === "frozen") {
    if (String(post.frozen_text || "").trim() !== frozen) {
      const err = new Error("post is frozen");
      err.status = 409;
      throw err;
    }
    return post;
